/* =========================================
   ContaEdu — Módulo: Conciliación Bancaria
   ========================================= */
...
const Conciliacion = (() => {
  ...
  const EJERCICIOS = [
    {
      ...
      saldoExt: 120000,
      saldoLib: 113500,
      contexto: 'Al 31/03 el extracto bancario muestra $120.000 y el libro de la empresa $113.500. Identificá a qué saldo afecta cada partida para conciliar.',
      partidas: [
        {
        ...
        },
        {
          ...
          afecta: 'ext',
          val: 2000,
          explicacion: 'La empresa lo depositó pero el banco aún no lo procesó. <strong>Suma al extracto</strong>.',
        },
        {
  ...
  function renderEjercicio() {
    ...
    const partidas = ej.partidas.map((p, i) => {
      const s = ejState[i] || {};
      const signo = p.val >= 0 ? '+' : '';
      const valLabel = `${signo}${Utils.formatARS(p.val)}`;
        ...
        <div class="check-row ${s.ok ? 'resolved' : ''}">
          <div style="flex:1">
            <div style="font-size:13px;margin-bottom:6px">
              ${p.desc}
              <span style="font-weight:500;color:${valColor};margin-left:4px">(${valLabel})</span>
            </div>
    ...
    if (completados > 0) {
      resumenHtml = `
        ...
        <div class="stat-grid">
          <div class="stat-card">
            <div class="stat-label">Extracto ajustado</div>
            <div class="stat-value">${Utils.formatARS(sExtAj)}</div>
          </div>
          <div class="stat-card">
            <div class="stat-label">Libro ajustado</div>
            <div class="stat-value">${Utils.formatARS(sLibAj)}</div>
          </div>
          <div class="stat-card">
            <div class="stat-label">Diferencia</div>
            <div class="stat-value" style="color:${dif < 1 ? 'var(--success)' : 'var(--danger)'}">
              ${dif < 1 ? 'Conciliado' : Utils.formatARS(dif)}
            </div>
          </div>
        </div>`;
      ...
      if (completados === ej.partidas.length) {
        resumenHtml += dif < 1
          ? Utils.alert('ok', 'trophy', `¡Conciliación correcta! Ambos saldos ajustados = ${Utils.formatARS(sExtAj)}`)
          : Utils.alert('err', 'alert-circle', `Diferencia de ${Utils.formatARS(dif)}. Revisá la clasificación de alguna partida.`);
      }
    }

    return `
      <div class="teoria">
        <strong>Conciliación bancaria:</strong> El saldo del libro puede diferir del extracto por: cheques emitidos no cobrados,
        depósitos en tránsito, notas de débito/crédito bancarias no registradas.
      ...
      <div class="card">
        ...
        <div class="stat-grid" style="margin-bottom:1rem">
          <div class="stat-card">
            <div class="stat-label">Saldo extracto bancario</div>
            <div class="stat-value">${Utils.formatARS(ej.saldoExt)}</div>
          </div>
          <div class="stat-card">
            <div class="stat-label">Saldo libro empresa</div>
            <div class="stat-value">${Utils.formatARS(ej.saldoLib)}</div>
          </div>
        </div>
    ...
    return `
      ...
      <div class="card">
        ...
        <div class="form-row">
          ...
          <div class="form-group">
            <label>&nbsp;</label>
            <button class="btn btn-primary" onclick="Conciliacion.agregarPartida()">
      ...
      <div class="card">
        ...
        ${dif < 1
          ? Utils.alert('ok', 'check', 'Conciliación correcta. Los saldos ajustados coinciden.')
