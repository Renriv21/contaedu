/* =========================================
   ContaEdu — Módulo: Asientos v2
   ========================================= */

const Asientos = (() => {

  const MODULO = 'asientos';
  ...
  const EJERCICIOS = [
    {
      ...
      pasos: [
        ...
        },
        {
          ...
          explicacion: '<strong>Caja</strong> aumenta → Debe. <strong>Ventas</strong> e <strong>IVA Ventas</strong> → Haber.',
        },
      ],
    },
    {
      ...
      contexto: '"El Sauce SRL" compra mercaderías a "López e Hijos" por $20.000 neto + IVA. Pago a 30 días.',
      pasos: [
        {
          titulo: 'Identificá las cuentas',
          tipo: 'opcion',
          consigna: '¿Qué cuentas intervienen?',
          opciones: ['Caja y Ventas', 'Mercaderías, IVA Compras y Proveedores', 'Banco y Capital', 'Clientes y Mercaderías'],
          correcta: 1,
          explicacion: 'Entra <strong>Mercaderías</strong>, pagamos <strong>IVA Compras</strong> y queda deuda con <strong>Proveedores</strong>.',
        },
        {
          titulo: 'Calculá los importes',
          tipo: 'calculo',
        ...
        },
        {
          ...
          explicacion: '<strong>Mercaderías</strong> e <strong>IVA Compras</strong> al Debe. <strong>Proveedores</strong> al Haber.',
        },
      ],
    },
    {
      ...
      pasos: [
        {
          ...
          explicacion: 'Aportes = 85.000 × 0,17 = <strong>$14.450</strong> / Neto = <strong>$70.550</strong>',
        },
        {
          titulo: 'Registrá el asiento',
          tipo: 'asiento',
  ...
  function renderLibre() {
    return `
      <div class="teoria">
        <strong>Partida doble:</strong> La suma del <strong>Debe</strong> siempre debe igualar la suma del <strong>Haber</strong>.
        En cuentas de Activo: lo que entra va al Debe, lo que sale al Haber. En Pasivo/PN es al revés.
      </div>
      <div class="card">
        ...
        <div class="dh-grid">
          <div class="dh-box dh-box-debe">
            ...
            ${[1,2].map(n=>`
              <div class="dh-row">
                ...
                <input type="text" id="d-c${n}" placeholder="Nombre de cuenta" style="width:100%;padding:7px 10px;border:1px solid var(--border-strong);border-radius:var(--radius-sm);font-family:var(--font-sans);font-size:12px;background:var(--bg-surface);color:var(--text-primary);margin-bottom:4px">
                <label>Importe $</label>
                <input type="number" id="d-m${n}" placeholder="0.00" min="0" style="width:100%;padding:7px 10px;border:1px solid var(--border-strong);border-radius:var(--radius-sm);font-family:var(--font-sans);font-size:12px;background:var(--bg-surface);color:var(--text-primary)">
              </div>`).join('')}
          </div>
          <div class="dh-box dh-box-haber">
            ...
            ${[1,2].map(n=>`
              <div class="dh-row">
                <label>Cuenta ${n}</label>
                <input type="text" id="h-c${n}" placeholder="Nombre de cuenta" style="width:100%;padding:7px 10px;border:1px solid var(--border-strong);border-radius:var(--radius-sm);font-family:var(--font-sans);font-size:12px;background:var(--bg-surface);color:var(--text-primary);margin-bottom:4px">
                <label>Importe $</label>
                <input type="number" id="h-m${n}" placeholder="0.00" min="0" style="width:100%;padding:7px 10px;border:1px solid var(--border-strong);border-radius:var(--radius-sm);font-family:var(--font-sans);font-size:12px;background:var(--bg-surface);color:var(--text-primary)">
              </div>`).join('')}
          </div>
        </div>
        <div style="margin-top:12px;display:flex;gap:10px">
          <button class="btn btn-primary" onclick="Asientos.registrar()">
            <i class="ti ti-device-floppy" aria-hidden="true"></i> Registrar
          </button>
  ...
  function renderEjercicio() {
    ...
    if (paso.tipo === 'asiento') {
      const inputStyle = 'width:100%;padding:7px 10px;border:1px solid var(--border-strong);border-radius:var(--radius-sm);font-family:var(--font-sans);font-size:12px;background:var(--bg-surface);color:var(--text-primary)';
  ...
  function renderDiario() {
    if (!libroDiario.length) {
      return `<div class="card"><p style="color:var(--text-muted);font-size:13px">Sin asientos registrados. Usá el <strong>Registro libre</strong> para agregar.</p></div>`;
    }
    return `
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem">
        <span style="font-size:13px;color:var(--text-secondary)">${libroDiario.length} asiento(s)</span>
        <button class="btn btn-sm" onclick="Asientos.limpiarDiario()"><i class="ti ti-trash" aria-hidden="true"></i> Limpiar</button>
      </div>` +
      libroDiario.map(a => `
        <div class="card" style="margin-bottom:10px">
          ...
          <div class="table-wrap">
            <table>
              ...
              <tbody>
                ${a.debe.map(d=>`<tr><td style="padding-left:16px">${d.cuenta}</td><td class="td-num">${Utils.formatARS(d.monto)}</td><td></td></tr>`).join('')}
                ${a.haber.map(h=>`<tr><td style="padding-left:32px">${h.cuenta}</td><td></td><td class="td-num">${Utils.formatARS(h.monto)}</td></tr>`).join('')}
                <tr class="td-total"><td>Total</td><td class="td-num">${Utils.formatARS(a.total)}</td><td class="td-num">${Utils.formatARS(a.total)}</td></tr>
              </tbody>
            </table>
          </div>
        </div>`).join('');
  ...
  function limpiar() {
    ...
    const msg=document.getElementById('as-msg'); if(msg) msg.innerHTML='';
  }

  function limpiarDiario() {
    if(confirm('¿Borrar todos los asientos?')){ libroDiario=[]; renderInner(); }
  }
  ...
  function verificarCalc() {
    ...
    if(rs.ok){ rs.mostradaExpl=true; _checkCompletar(); }
    renderInner();
  }
  ...
  /* Guarda progreso si es el último paso */
  function _checkCompletar() {
    const ej=EJERCICIOS[ejActual];
    if(pasoActual===ej.pasos.length-1){
      Progreso.completar(MODULO, ejActual);
      App.refrescarProgreso();
    }
  }

  function verExpl()  { respuestas[pasoActual].mostradaExpl=true; renderInner(); }
